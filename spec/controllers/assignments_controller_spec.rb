...
RSpec.describe AssignmentsController, type: :controller do
  ...
  describe "post #show" do
    ...
    context "when valid date range" do
      ...
      Given!(:assignment) { FactoryGirl.create(:assignment, deliverer_id: deliverer.id, shift_id: shift.id) }
      ...
      context "initializes shifts within range" do
        ...
        # Match Shift info
        And { expect(response.body).to match("#{shift.start_time_to_s}") }
        And { expect(response.body).to match("#{shift.end_time_to_s}") }
        And { expect(response.body).to match("#{shift.deliverers.count}/#{shift.max_count}") }

        # Match Deliverer info within Shift
        And { expect(response.body).to match("#{deliverer.id}") }
